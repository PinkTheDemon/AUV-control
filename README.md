...
修改了训练的动作维度，把原来的12维中已知为0的维度去掉，剩下4维，另外把折扣因子从原来的0.95修改为0.9，得到了收敛的结果。虽然训练效果还不够好，但是可能是训练次数不够，因为现在设置的是连续10次能够跑完2000步就停止训练。测试效果如下：

![](https://z4a.net/images/2023/07/14/7-14-195.png)
...
上面是10集跑完就停止，可以看到停止时平均奖励还很小，下面是设置平均奖励大于-10万才停止，停止时效果如下：（可惜这个结果的policy忘记存了）

![](https://z4a.net/images/2023/07/17/7-14.png)

![](https://z4a.net/images/2023/07/17/7-14-2w.png)

下面是设置平均奖励大于负1万停止，停止时效果如下：（agentData714(3)）

![](https://z4a.net/images/2023/07/17/7-149dd6f3f7210f9f56.png)

![](https://z4a.net/images/2023/07/17/7-14-8k.png)

可以看到，平均奖励更小的收敛结果，在测试时更稳定，但却有更大的稳态误差，当然稳态误差也是在人为补偿下就存在的。平均奖励更小意味着更接近人为补偿，不清楚是好还是不好。在不存在模型偏差的情况下，theta只是有轻微波动，但没有稳态误差

把关于误差项的参数设置注释掉了（变回默认参数），然后学到了平均奖励大于负5k的策略，但是效果怎么感觉更差了，稳态误差也变大了（agentData715.mat）

![](https://z4a.net/images/2023/07/17/7-155k.png)
